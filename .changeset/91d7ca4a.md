...
Implement the OpenCode usage-limits TUI plugin as a full package entrypoint.
...
- Added prompt-footer usage that detects the active OpenCode session provider and shows compact Codex 5h usage for OpenAI sessions or ZAI token usage for ZAI Coding Plan sessions.
