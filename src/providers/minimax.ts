...
/**
 * Resolves and validates the MiniMax base URL before it reaches a request.
 *
 ...
 * international default so the subscription key is never sent to an unexpected
 * host.
 *
 * @param baseUrl - Configured base URL, or `undefined` for the default.
 * @returns A safe, absolute URL string with no trailing slash.
 */
const resolveMiniMaxBaseUrl = (baseUrl: string | undefined): string => {
  const fallback = DEFAULT_MINIMAX_BASE_URL.replace(/\/$/u, "");
  let parsed: URL;
  try {
    parsed = new URL((baseUrl ?? DEFAULT_MINIMAX_BASE_URL).trim());
  } catch {
    return fallback;
  }
 ...
 *
 * The endpoint returns an object with a `model_remains` array (one record per
 * model) and a `base_resp` envelope carrying `status_code` and `status_msg`.
 * A non-zero `status_code` or a missing `model_remains` array is treated as an
 * invalid response.
 *
